...
static FmDemodContext* g_ctx = NULL;
...
static void rtlsdr_callback(unsigned char* buf, uint32_t len, void* userctx)
{
    ...
    fm_demod_process(g_ctx, g_ctx->h_raw, g_ctx->h_audio);
...
static void run_bench(FmDemodContext* ctx, const char* path, uint32_t samp_rate)
{
    ...
    fm_demod_process(ctx, ctx->h_raw, ctx->h_audio);
...
int main(int argc, char** argv)
{
    ...
    while ((opt = getopt(argc, argv, "f:r:s:o:t:b:")) != -1) {
        ...
        default:
            fprintf(stderr, "Usage: fm_rx [-f freq_hz] [-r sample_rate] [-s offset_hz] [-o output.wav] [-t duration_sec] [-b bench_file.bin]\n");
            return 1;
        }
    }
    ...
    if (bench_path) {
        ...
        FmDemodContext* ctx = fm_demod_create(BLOCK_SIZE, DECIM1, taps1, FIR_TAPS1,
    ...
    fprintf(stderr, "Warming up GPU...\n");
    g_ctx = fm_demod_create(BLOCK_SIZE,
