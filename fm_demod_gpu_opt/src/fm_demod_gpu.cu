    ...
    float prev_q)
{
    ...
    float i1 = s_i[threadIdx.x + 1];
    float q1 = s_q[threadIdx.x + 1];
    float i0 = s_i[threadIdx.x];
    float q0 = s_q[threadIdx.x];
    ...
    float freq_shift_hz, float sample_rate)
{
    ...
    CUDA_CHECK(cudaMalloc(&ctx->d_dc_sum, sizeof(float)));
    ...
    CUDA_CHECK(cudaMemsetAsync(ctx->d_dc_sum, 0, sizeof(float), s));

    kernel_normalize_and_shift<<<(B+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_raw, ctx->d_i, ctx->d_q, B, 0.0f, 0.0f);
    kernel_decimate_iq<<<(C+THREADS-1)/THREADS, THREADS, smem_iq1, s>>>(ctx->d_i, ctx->d_q, ctx->d_i1, ctx->d_q1, ctx->d_taps1, n_taps1, decim1, B);
    kernel_fm_discriminate<<<(C+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_i1, ctx->d_q1, ctx->d_discrim, C, 1.0f, 0.0f);
    kernel_dc_reduce<<<dc_blocks, THREADS, THREADS * sizeof(float), s>>>(ctx->d_discrim, ctx->d_dc_sum, C);
    kernel_dc_subtract<<<(C+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_discrim, ctx->d_dc_sum, C);
    kernel_decimate<<<(A+THREADS-1)/THREADS, THREADS, smem2, s>>>(ctx->d_discrim, ctx->d_audio, ctx->d_taps2, n_taps2, decim2, C);
...
extern "C" void fm_demod_destroy(FmDemodContext* ctx)
{
    ...
    cudaFree(ctx->d_dc_sum);
    cudaStreamDestroy(ctx->stream);
    free(ctx);
}

extern "C" void fm_demod_process(FmDemodContext* ctx)
{
    ...
    // 1. Normalize IQ + frequency shift
    kernel_normalize_and_shift<<<(B+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_raw, ctx->d_i, ctx->d_q, B, ctx->phase_inc, ctx->phase);

    // 2. Channel filter + decimate I and Q
    kernel_decimate_iq<<<(C+THREADS-1)/THREADS, THREADS, smem_iq1, s>>>(ctx->d_i, ctx->d_q, ctx->d_i1, ctx->d_q1, ctx->d_taps1, ctx->n_taps1, ctx->decim1, B);

    // 3. FM discriminator
    kernel_fm_discriminate<<<(C+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_i1, ctx->d_q1, ctx->d_discrim, C, ctx->last_i, ctx->last_q);

    // 4. DC block: zero the accumulator, reduce, then subtract
    CUDA_CHECK(cudaMemsetAsync(ctx->d_dc_sum, 0, sizeof(float), s));
    kernel_dc_reduce<<<dc_blocks, THREADS, THREADS * sizeof(float), s>>>(ctx->d_discrim, ctx->d_dc_sum, C);
    kernel_dc_subtract<<<(C+THREADS-1)/THREADS, THREADS, 0, s>>>(ctx->d_discrim, ctx->d_dc_sum, C);

    // 5. Audio filter + decimate
    kernel_decimate<<<(A+THREADS-1)/THREADS, THREADS, smem2, s>>>(ctx->d_discrim, ctx->d_audio, ctx->d_taps2, ctx->n_taps2, ctx->decim2, C);
    ...
    // Copy last channel-decimated IQ sample for next block's discriminator boundary
    if (C > 0) {
        CUDA_CHECK(cudaMemcpy(&ctx->last_i, ctx->d_i1 + (C - 1), sizeof(float), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(&ctx->last_q, ctx->d_q1 + (C - 1), sizeof(float), cudaMemcpyDeviceToHost));
    }
}
